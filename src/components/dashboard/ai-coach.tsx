...
export const AICoach = ({ data = [] }: AICoachProps) => {
  // Logic: Calculate real metrics for the insights
  const totalSpent = data?.reduce((acc, curr) => acc + curr.amount, 0) || 0;
  const foodSpending = data?.filter(t => t.category.toLowerCase() === 'food')
  ...
  return (
    <div>
      ...
      <div className="bg-surface-cards border border-border rounded-[5px] p-6 h-full mt-8 shadow-md shadow-[#7700FF38]">
        ...
        <div className="flex flex-col gap-4">
          {/* Dynamic Warning: Checks if Food spending is high */}
          <InsightCard
            type="warning"
            text={`You've spent RM ${foodSpending.toFixed(2)} on Food so far. This is your highest expense category this month.`}
          />
          
          {/* Dynamic Success/Tip: Checks budget status */}
          <InsightCard
            type={isOverBudgetRisk ? "warning" : "success"}
            text={isOverBudgetRisk 
              ? `Careful! You've used ${((totalSpent/budgetLimit)*100).toFixed(0)}% of your budget. Slow down to last until the 31st.`
              : "Great job! You are currently under your budget limit and spending at a sustainable pace."
            }
          />

          {/* Dynamic Tip: Based on transaction count */}
          <InsightCard
            type="tip"
            text={`You have recorded ${data?.length || 0} transactions this month. Keep tracking to maintain a clear financial overview.`}
          />
        </div>

        {/* Keeping your exact button design */}
