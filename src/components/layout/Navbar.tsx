...
const Navbar = () => {
  ...
  const router = useRouter();
  const supabase = createClient();
  const [user, setUser] = useState<User | null>(null); // NEW: Track actual authentication state
  const [avatarUrl, setAvatarUrl] = useState<string>("/images/avatar.jpg");

  useEffect(() => {
    const fetchUserAndAvatar = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      setUser(user); // Save user instance
      if (user?.user_metadata?.avatar_url) {
        setAvatarUrl(user.user_metadata.avatar_url);
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setUser(session?.user || null); // Sync user instance on auth changes
      if (session?.user?.user_metadata?.avatar_url) {
        setAvatarUrl(session.user.user_metadata.avatar_url);
      } else {
        setAvatarUrl("/images/avatar.jpg");
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 md:px-10 lg:px-14 py-6">
        ...
        <div className="hidden md:flex items-center gap-3 relative">
          {user ? (
            ...
            <Link 
              href="/login" 
              className="flex items-center gap-2 py-1.5 px-4 bg-blue hover:bg-hover text-white rounded-lg text-sm transition-all active:scale-95 font-medium"
            >
              <LogIn size={14} />
              Sign In
            </Link>
            ...
            >
              ...
              {user ? (
                ...
                <SheetClose asChild>
                  <Link
                    href="/login"
                    className="absolute bottom-10 left-8 flex items-center gap-3 border-t border-gray-800 pt-8 w-52.5 text-left focus:outline-none text-white font-medium hover:text-secondary transition-colors"
                  >
